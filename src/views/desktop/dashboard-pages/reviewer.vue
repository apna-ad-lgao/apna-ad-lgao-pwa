<template>
  <section class="dashboard-reviewer-page h-min-screen mt-12" @scroll="handleScroll">
    ...
    <div v-else>
      ...
      >
        <div class="flex justify-between xl:h-40 lg:h-32 h-24 leading-tight m-5 text-center">
          <!-- <button class="p-8 rounded bg-gray-100 border border-gray-300"> -->
          <div
            class="rounded bg-gray-100 border border-gray-400 flex flex-col align-center justify-center xl:w-48 lg:w-40 w-32 pointer"
            @click.prevent="applyBtnFilters('total')"
          >
            <!-- <h4 class="text-3xl font-bold">
              {{ companies ? companies.length : 0 }}
            </h4> -->
            <h4 class=" lg:text-4xl text-3xl xl:text-5xl font-bold">
              {{ totalAccounts }}
            </h4>
            <p class="text-xs lg:text-sm xl:text-lg mt-2 mx-2 text-gray-700 font-bold letter-spacing-05 uppercase">
              Total Accounts
            </p>
            <br>
          </div>
          <div
            ...
            class="relative stats-cont rounded bg-gray-100 border border-gray-400 flex flex-col
            align-center justify-center xl:w-48 lg:w-40 w-32 pointer transition-normal"
            @click.prevent="applyBtnFilters('accountsInred')"
          >
            <!-- @click.prevent="applyBtnFilters('accountsInred')" -->
            <h4 class=" lg:text-4xl text-3xl xl:text-5xl text-red-600 font-bold">
              {{ totalAccountsInRed }}
            </h4>
            <p class="text-xs lg:text-sm xl:text-lg mt-2 mx-2 text-gray-700 font-bold letter-spacing-05 uppercase">
              Accounts In Red
        ...
        <div class="flex justify-center mx-5 text-gray-600 text-lg">
        ...
        <div
          ...
          class="mx-5 my-3 text-gray-600 items-center lg:flex"
        >
          <div class="lg:flex items-center">
            ...
            <div class="flex my-2">
              <span v-if="query" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  Filter query: <strong>{{ query }}</strong>
                </label>
              </span>
              <span v-if="typeFilter" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  Account Type: <strong>{{ typeFilter }}</strong>
                </label>
              </span>
              <span v-if="verticalFilter" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  Vertical: <strong>{{ verticalFilter }}</strong>
                </label>
              </span>
              <!-- <span v-if="statusFilter" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  Status: {{ statusFilter?'In-active':'Active' }}
                </label>
              </span> -->
              <span v-if="filterAccountsInRed" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  {{ totalAccountsInRed }} account{{ totalAccountsInRed > 1 ? 's' : '' }} in Red
                </label>
              </span>
              <span v-if="filterAccountsInRedVoc" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
                  VOS {{ totalAccountsInRedVoc }} account{{ totalAccountsInRedVoc > 1 ? 's' : '' }} in Red
                </label>
              </span>
              <span v-if="filterAccountsInRedVod" class="text-gray-600 mx-2 my-1">
                <label class="border border-gray-400 bg-gray-300 inline-block px-2 pointer rounded">
      ...
      <div class="bg-gray-100 border border-gray-400 flex justify-center items-center px-3 py-1 mb-5 md:mx-0 mx-3 rounded text-gray-700">
        <p class="">
          Showing Accounts <b>{{ filteredResults.length > 1 ? '' : '' }}: {{ filteredResults.length }}</b>
        </p>
        <button
      ...
      <section class="bg-white relative">
        ...
        >
          ...
          <div class="w-2/3 flex justify-between items-center">
            ...
            <p class="w-1/4 text-center none lg:block">
              Type
            </p>
            <p class="w-1/4 text-center">
              VOS<!--  <span class="text-xs text-red-500">(Rating<i class="far fa-star" />)</span> -->
            </p>
            <p class="w-1/4 text-center">
              VOD<!--  <span class="text-xs text-red-500">(Rating<i class="far fa-star" />)</span> -->
            </p>
          </div>
        </div>
        ...
        <section v-else class="px-4">
          <router-link
            ...
            "
          >
            <div class="w-1/3 leading-snug text-lg">
              <p>{{ company.name }}</p>
              <ul class="flex">
                <li class="lg:hidden flex border border-b-4 border-blue-600 font-bold px-2 shadow-lg rounded text-black text-blue-600 text-sm">
                  {{ company.type }}
                </li>
              </ul>
            </div>
            ...
            <div class="flex justify-between items-center w-2/3">
              ...
              <div class="w-1/4 text-center none lg:block">
                {{ company.type }}
              </div>
              <div class="no-text-wrap  flex items-center justify-center w-1/4">
                ...
                <span v-else><i class="far fa-star text-gray-500" /><i class="far fa-star text-gray-500" />
                  <i class="far fa-star text-gray-500" /><i class="far fa-star text-gray-500" />
                  <i class="far fa-star text-gray-500" /></span>
              </div>
              <div class="no-text-wrap flex items-center justify-center w-1/4">
