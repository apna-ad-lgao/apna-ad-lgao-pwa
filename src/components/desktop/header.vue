<template>
  <header class="light" :data-open-state="openState">
    ...
    <Slide :no-overlay="false" :burger-icon="false" :is-open="menuOpened" class="burger-menu-cont" @closeMenu="menuOpened = false">
      ...
      <router-link
        v-if="(user.isVOD || user.isVOC) && (user.isReviewer)"
        to="/dashboard/tasks"
        class="text-lg letter-spacing-05 uppercase font-normal mt-2 ml-2"
      >
        <span>Input Mode</span>
      </router-link>
      <router-link v-if="user.isReviewer || user.isAdmin" to="/dashboard/review" class="text-lg letter-spacing-05 uppercase font-normal mt-2 ml-2">
      ...
      <router-link
        v-if="user.isReviewer || user.isAdmin"
